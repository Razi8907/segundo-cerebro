...
// POST /api/data/proveedor-seguimiento/upload-url
// Body: { country, filename } → { path, signedUrl, token }
export async function POST(req: NextRequest) {
  ...
  const filename = (body.filename || "proveedores.xlsx").replace(/[^a-zA-Z0-9._-]/g, "_");
  if (!["ar", "py"].includes(country)) return NextResponse.json({ error: "country inválido" }, { status: 400 });

  const supabase = getSupabase();
  const path = `proveedor-seg/${country}/${Date.now()}-${filename}`;
