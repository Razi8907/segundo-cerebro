...
export default function ArgentinaError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[Argentina Error]:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-screen p-6" style={{ background: "var(--bg-page)" }}>
      <div className="max-w-3xl mx-auto rounded-xl p-6 border border-red-500/40" style={{ background: "var(--bg-card)" }}>
        <h1 className="text-lg font-bold text-red-400 mb-3">⚠️ Error en Argentina Dashboard</h1>
