...
export default function MinimoMensual({ country, mes }: { country: "ar" | "py"; mes: Mes }) {
  ...
  const mesAntAnt: Mes | null = mes === "septiembre" ? "julio" : mes === "agosto" ? "junio" : mes === "julio" ? "mayo" : mes === "junio" ? "abril" : null;
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const map: Record<Mes, ByDS[]> = { abril: [], mayo: [], junio: [], julio: [], agosto: [], septiembre: [] };
      const dailyMap: Record<Mes, ByDSDaily[]> = { abril: [], mayo: [], junio: [], julio: [], agosto: [], septiembre: [] };
      const byDateMap: Record<Mes, ByDate[]> = { abril: [], mayo: [], junio: [], julio: [], agosto: [], septiembre: [] };
      meses.forEach((m, i) => {
  ...
  const isCurrent = today.getFullYear() === 2026 && (today.getMonth() + 1) === ({ abril: 4, mayo: 5, junio: 6, julio: 7, agosto: 8, septiembre: 9 }[mes]);
